...
HELP_TEXT = (
    ...
    "/arena_punishment — проверить арену жизни\n\n"
    "❓ <b>Нужна помощь?</b>\n"
...
async def guide_callback(cb: types.CallbackQuery) -> None:
    guide_text = (
        ...
        "• Используйте /arena_punishment для проверки дисциплины\n"
        "• Настройте часовой пояс командой /timezone"
    )
...
async def finance_creditors(cb: types.CallbackQuery) -> None:
    ...
    async with session_scope() as session:
        ...
        if not creditors:
            await cb.message.edit_text(
                        "💸 <b>Кредиторы</b>\n\n"
        "Кредиторы - это люди/организации, которым ВЫ должны деньги.\n\n"
        ...
        "Нажмите кнопку ниже, чтобы добавить первого.",
            ...
            text = "💸 <b>Ваши кредиторы (кому ВЫ должны):</b>\n\n"
...
async def finance_debtors(cb: types.CallbackQuery) -> None:
    ...
    async with session_scope() as session:
        ...
        if not debtors:
            await cb.message.edit_text(
                        "🏦 <b>Должники</b>\n\n"
        "Должники - это люди/организации, которые должны деньги ВАМ.\n\n"
        ...
        "Нажмите кнопку ниже, чтобы добавить первого.",
            ...
            text = "🏦 <b>Ваши должники (кто должен ВАМ):</b>\n\n"
