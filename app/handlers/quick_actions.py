...
from app.db.models import User, GeneralThought, Book, BookThought
...
async def quick_quote_author_handler(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        async with session_scope() as session:
            ...
            # Создаем цитату без привязки к книге (общая цитата)
            # TODO: Создать модель GeneralQuote для сохранения общих цитат
            ...
            await message.answer(
                ...
                "Цитата записана. В будущем она будет сохраняться в базе данных.",
                reply_markup=back_main_menu(),
                parse_mode="HTML"
            )
...
async def quick_thought_handler(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        async with session_scope() as session:
            ...
            from app.db.models.book import GeneralThought
            
            # Создаем общую мысль
            new_thought = GeneralThought(
                user_id=db_user.id,
...
@router.callback_query(F.data == "view_thoughts")
async def view_thoughts_handler(cb: types.CallbackQuery) -> None:
    """Показать все мысли пользователя"""
    user = cb.from_user
    
    async with session_scope() as session:
        db_user = (await session.execute(select(User).where(User.telegram_id == user.id))).scalar_one()
        
        # Получаем общие мысли
        general_thoughts = (await session.execute(
            select(GeneralThought).where(GeneralThought.user_id == db_user.id).order_by(GeneralThought.created_at.desc())
        )).scalars().all()
        
        # Получаем мысли из книг
        book_thoughts = (await session.execute(
            select(BookThought).join(Book).where(Book.user_id == db_user.id).order_by(BookThought.created_at.desc())
        )).scalars().all()
        
        # Объединяем все мысли
        all_thoughts = []
        
        # Добавляем общие мысли
        for thought in general_thoughts:
            all_thoughts.append({
        ...
        # Добавляем мысли из книг
        for thought in book_thoughts:
            all_thoughts.append({
                'text': thought.thought_text,
                'type': thought.thought_type,
                'date': thought.created_at,
        ...
        # Сортируем по дате
        all_thoughts.sort(key=lambda x: x['date'], reverse=True)
    
    if not all_thoughts:
        await cb.message.edit_text(
            "💭 <b>Ваши мысли</b>\n\n"
            "У вас пока нет сохраненных мыслей.\n"
            "Запишите первую мысль!",
            reply_markup=back_main_menu(),
            parse_mode="HTML"
        )
        await cb.answer()
    ...
    # Формируем текст с мыслями
    thoughts_text = f"💭 <b>Ваши мысли</b>\n\n"
    thoughts_text += f"Всего мыслей: {len(all_thoughts)}\n\n"
    
    for i, thought in enumerate(all_thoughts[:10], 1):  # Показываем первые 10
        thoughts_text += f"<b>{i}.</b> {thought['text'][:100]}{'...' if len(thought['text']) > 100 else ''}\n"
        
        if thought['source'] == 'book':
            thoughts_text += f"   📚 Книга: {thought['book_title']}\n"
        
        thoughts_text += f"   🏷️ Тип: {thought['type']}\n"
        thoughts_text += f"   📅 {thought['date'].strftime('%d.%m.%Y %H:%M')}\n\n"
    
    if len(all_thoughts) > 10:
        thoughts_text += f"... и еще {len(all_thoughts) - 10} мыслей\n\n"
    
    # Создаем клавиатуру для навигации
    keyboard = []
    if len(all_thoughts) > 10:
        keyboard.append([
            InlineKeyboardButton(text="📄 Показать все", callback_data="view_all_thoughts")
        ])
...
@router.callback_query(F.data == "view_all_thoughts")
async def view_all_thoughts_handler(cb: types.CallbackQuery) -> None:
    """Показать все мысли с пагинацией"""
    user = cb.from_user
    ...
    thoughts_text = f"💭 <b>Все ваши мысли</b>\n\n"
    thoughts_text += f"Всего мыслей: {len(all_thoughts)}\n\n"
    
    for i, thought in enumerate(all_thoughts, 1):
        thoughts_text += f"<b>{i}.</b> {thought['text']}\n"
    ...
    # Создаем клавиатуру
    keyboard = [[InlineKeyboardButton(text="⬅️ Назад к краткому виду", callback_data="view_thoughts")]]
...
async def quick_reminder_time_handler(cb: types.CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        # Используем системное время устройства пользователя
        now_user = datetime.now()
        if time_option == "1h":
            reminder_time = now_user + timedelta(hours=1)
            time_display = "через 1 час"
        elif time_option == "3h":
            reminder_time = now_user + timedelta(hours=3)
            time_display = "через 3 часа"
        elif time_option == "6h":
            reminder_time = now_user + timedelta(hours=6)
            time_display = "через 6 часов"
        elif time_option == "12h":
            reminder_time = now_user + timedelta(hours=12)
            time_display = "через 12 часов"
        elif time_option == "tomorrow":
            reminder_time = now_user.replace(hour=9, minute=0, second=0, microsecond=0) + timedelta(days=1)
            time_display = "завтра утром в 9:00"
        elif time_option == "week":
            reminder_time = now_user + timedelta(weeks=1)
...
async def quick_reminder_custom_time_handler(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        patterns = [
            # Через X часов
            (r'через (\d+) час(?:а|ов)?', lambda m: now_user + timedelta(hours=int(m.group(1))), 
             lambda m: f"через {m.group(1)} час(а/ов)"),
            
            # Через X минут
            (r'через (\d+) минут(?:у|ы)?', lambda m: now_user + timedelta(minutes=int(m.group(1))), 
             lambda m: f"через {m.group(1)} минут(у/ы)"),
            
            # В HH:MM (сегодня)
            (r'в (\d{1,2}):(\d{2})', lambda m: now_user.replace(hour=int(m.group(1)), minute=int(m.group(2)), second=0, microsecond=0), 
             lambda m: f"в {m.group(1)}:{m.group(2)}"),
            
            # Завтра в HH:MM
            (r'завтра в (\d{1,2}):(\d{2})', lambda m: (now_user + timedelta(days=1)).replace(hour=int(m.group(1)), minute=int(m.group(2)), second=0, microsecond=0), 
             lambda m: f"завтра в {m.group(1)}:{m.group(2)}"),
            
            # Через X дней
            (r'через (\d+) дн(?:ень|я|ей)?', lambda m: now_user + timedelta(days=int(m.group(1))), 
             lambda m: f"через {m.group(1)} дн(ень/я/ей)"),
            
            # В X часов (сегодня)
            (r'в (\d{1,2}) час(?:а|ов)?', lambda m: now_user.replace(hour=int(m.group(1)), minute=0, second=0, microsecond=0), 
        ...
        if reminder_time and reminder_time <= now_user:
